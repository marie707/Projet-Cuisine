...
;$(function(){$("#input-rating").rating({min:0,max:5,step:1}).on('rating.change',function(event,value){$.post(url_notation,{id:id_object,rate:value},function(db)
{if(type_vote=="courslive"){$('.sidebar').css('height','380px');$('.sidebar .row-fluid:last-child').hide();$("#notationBox").slideUp();$('#avisCourslive').show();}
else if(type_vote=="technique"){if(CONFIG.lang=="fr")
var message_vote="Merci pour votre vote! Vous avez mis la note de "+value+" à cette technique";else
var message_vote="Thanks for your vote! You gave a "+value+" to this technical";$('#message_vote').empty().append('<span style="color:#fff;">'+message_vote+'</span>');if(db.votes!=null)
$("#all_votes").text(" ("+db.votes);}
else{$("#all_votes").text(" ("+db.votes);if(CONFIG.lang=="fr")
var message_vote="Merci pour votre vote! Vous avez mis la note de "+value+" à cette recette";else
var message_vote="Thanks for your vote! You gave a "+value+" to this recipe";$('#message_vote').empty().append('<span style="color:#4d3834;">'+message_vote+'</span>');}},"json");});});